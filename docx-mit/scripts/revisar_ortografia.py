#!/usr/bin/env python3
"""
Revisa la ortografía del texto de un .docx y SEÑALA posibles faltas para que el modelo
las vea y las corrija antes de entregar. Extrae el texto con python-docx y aplica
heurísticas para el español (foco Ecuador): mojibake, tildes perdidas, terminaciones
-cion/-sion sin tilde y una lista de palabras que sin tilde no existen. Luego imprime el
texto completo para una revisión visual. Implementación propia (MIT).

Uso:
    python -X utf8 revisar_ortografia.py documento.docx

Correr SIEMPRE con `-X utf8` en Windows (si no, la consola cp1252 rompe los acentos).

Es un ASISTENTE, no un corrector infalible: marca sospechosos y muestra el texto; la
decisión final (y leer el resto en busca de errores) es del modelo.
"""
import re
import sys
import unicodedata

# Palabras cuya forma SIN tilde no es una palabra válida del español -> casi seguro falta la tilde.
SIEMPRE_TILDE = {
...
# Marcadores de mojibake (UTF-8 mal decodificado) frecuentes.
MOJIBAKE = ["Ã", "Â", "Ð", "�", "â€", "Ã³", "Ã±", "Ã©", "Ã¡", "Ãº", "Ã­"]

# Vocal acentuada colapsada a mayúscula dentro de palabra: "posiciAn", "funciOn".
VOCAL_MOJI = re.compile(r"[a-záéíóúñ][AEIOU][a-záéíóúñ]")

# Sustantivos terminados en consonante+"ion" en singular SIEMPRE llevan tilde en la ó
# (-ción/-sión/-tión/-xión/-gión/-nión...). Excluye el plural -iones (no está a fin de
# palabra) y "guion"/"ion" (la "ion" va precedida de vocal, no de consonante).
SUFIJO_ION = re.compile(r"\b[a-zñ]*[bcdfghjklmnpqrstvwxyz]ion\b", re.IGNORECASE)

TOKEN = re.compile(r"[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+")


def _sin_tilde(w: str) -> str:
    return "".join(c for c in unicodedata.normalize("NFD", w) if unicodedata.category(c) != "Mn")


def revisar(texto: str):
    """Devuelve lista de (renglón, hallazgo)."""
    hallazgos = []
    for n, linea in enumerate(texto.splitlines(), 1):
        ...
        for m in VOCAL_MOJI.finditer(linea):
            # evitar siglas/camelCase de una sola mayúscula legítima es difícil; en prosa española es casi siempre mojibake
...
def main():
    if len(sys.argv) != 2:
        print("uso: python -X utf8 revisar_ortografia.py <documento.docx>")
        sys.exit(1)
